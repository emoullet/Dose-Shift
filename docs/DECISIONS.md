...
The draft setup displays the controlled medication identities, formulations, doses, and timing windows read-only. Clinician/pharmacist validation is shown only as a prerequisite and is neither recorded nor claimed. Creation preserves every existing study and atomically refuses a second `draft` or `active` study.
